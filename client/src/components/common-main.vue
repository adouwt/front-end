<template>
  <div>
    ...
    <h5>初始化访问量</h5>
    <p>{{viewsCount}} 直接使用views {{this.$store.state.views}}</p>
    <h4>修改访问量 </h4>
    <span>{{this.$store.state.blogNumber}}</span>
    <h4>总计</h4>
...
export default {
  ...
  created () {
    ...
    this.viewsAdd() // 通过mapActions 触发mutation 从而commit ，改变state的值
  },
  computed: {
    ...mapState({
      viewsCount: 'views'
    ...
    ...mapActions({
      viewsAdd: 'addViews' // viewsAdd是定义的一个函数别名称，挂载在到this(vue)实例上，addViews 才是actions里面函数方法名称
    }) 
  },
  methods: {
    ...mapMutations({
